...
const char * const BITCOIN_CONF_FILENAME = "dak.conf";
const char * const BITCOIN_PID_FILENAME = "dakd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "dak" is a composite category enabling all Dak-related debug output
                if(ptrCategory->count(std::string("dak"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DakCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DakCore
    // Mac: ~/Library/Application Support/DakCore
    // Unix: ~/.dakcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DakCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DakCore";
#else
    // Unix
    return pathRet / ".dakcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty dak.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override dak.conf
