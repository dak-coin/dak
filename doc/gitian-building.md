...
*Setup instructions for a Gitian build of Dak Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Dak
...
- [Building Dak Core](#building-dak-core)
...
Clone the git repositories for Dak Core and Gitian.
...
in the Dak Core repository under 'Fetch and create inputs' to install sources which require
...
To build Dak Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Dak Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dak/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/dakpay/dak
...
For example:
```bash
URL=https://github.com/crowning-/dak.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dak=${COMMIT} --url dak=${URL} ../dak/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dak=${COMMIT} --url dak=${URL} ../dak/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dak=${COMMIT} --url dak=${URL} ../dak/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the dak git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../dak/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/dak
SIGPATH=/some/root/path/dak-detached-sigs

./bin/gbuild --url dak=${BTCPATH},signature=${SIGPATH} ../dak/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dak-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dak-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/dak-osx-build.assert
```
...
[dak/gitian.sigs](https://github.com/dakpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
